...
def features_construct_graph_new(features, k=15, pca=None, mode="connectivity", metric="cosine"): #自己更改的
    print("start features construct graph")
    if pca is not None:
        features = dopca(features, dim=pca).reshape(-1, 1)
    ...
    # 构建负样本邻接矩阵 graph_neg
    graph_neg = torch.ones(features.shape[0], features.shape[0]) - graph_nei

    return fadj,# graph_nei, graph_neg
