...
def scMultiGAN(args, data_gen, data_critic, data_loader):
    ...
    with open('training_log.txt', 'a') as file:
        for epoch in range(start_epoch, epochs):
            ...
            for i,real_sample in enumerate(data_loader):
                ...
                #masked_real_data = real_data * gene_mask
                masked_real_data = real_data
                data_noise.normal_()
                mask_noise.normal_()
                #print(data_noise.dtype)
                fake_data = data_gen(data_noise,label_hot)
                #fake_data = fake_data * gene_mask
                ...
                if critic_updates == n_critic:
                    ...
                    #fake_mask = mask_gen(mask_noise,label_hot)
                    #masked_fake_data = mask_data(fake_data, fake_mask, tau)
                    #masked_fake_data = fake_data * gene_mask
                    masked_fake_data = fake_data
                    ...
                    print("\r[Epoch %d/%d] [Batch %d/%d] [G_data loss: %f] [G_mask loss: %f]" % (
                        epoch + 1, args.epoch, i + 1, len(data_loader),
                        data_loss.detach().cpu().numpy().item(),(data_loss).detach().cpu().numpy().item()))
            mean_data_loss = sum_data_loss / n_batch
            #mean_mask_loss = sum_mask_loss / n_batch
            print("epoch {} data_loss is {}".format(epoch + 1, mean_data_loss))
            log_message = "Epoch {} data loss is {}\n".format(epoch + 1, mean_data_loss)
            print(log_message.strip())
            file.write(log_message)  # 
            if save_interval > 0 and (epoch + 1) % save_interval == 0 and epoch > 190:
